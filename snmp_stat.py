...
def snmp_set_operations(escId, opState, siteId, entry, adminState):
        print("\t\t\t\t\t\t[SNMP] Set operations table")
        setDB_int((operational + str(1) + "." + str(entry)), escId);
        setDB_str((operational + str(2) + "." + str(entry)), siteId);
        setDB_str((operational + str(3) + "." + str(entry)), opState);
        setDB_str((operational + str(4) + "." + str(entry)), adminState);
...
def handle_response(data):
    ...
    if data:
        ...
        print 'Push data into Queue {:s} {:s}'.format(escId, json_parse)
        escQ.put_data_into_queue(escId,json_parse)
