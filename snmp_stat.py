...
def snmp_handler_response(data):
    global create_flag
    index_counter = 0

    log.info("(rest_server) Waiting for data...")
...
def snmp_stat_handler():
    ...
    while True:
        ...
        try:
            t2 = threading.Thread(name='csv_client', target=csv_client_handler, args=(connection, e, 2))
            t2.start()
            log.info(connection)
